...
Shiny.inputBindings.register((function () {
    ...
    input_binding.find = function (scope) {
        // Scope may be jQuery for a modal dialog
        return $(scope)[0].querySelectorAll('.hodfr');
    };
    ...
    input_binding.receiveMessage = function (el, data) {
        ...
        if (data.hasOwnProperty('render')) {
            // https://forum.handsontable.com/t/hot-instances-on-an-initially-hidden-tab-dont-render-until-clicked/1864
            // https://stackoverflow.com/questions/42492264/handsontable-not-rendering-all-columns-until-right-mouse-click
            $(el).data('hodf').hot.render();
        }
